---
title: "In-Process Key-Value Stores for Local State"
sidebarTitle: "Local Stores"
description: "Share fast in-memory state across all Swoole workers on a host using Winter Boot's built-in KV store and Queue store — no external infrastructure required."
---
...
<Info>
  Both stores are **node-local**. All Swoole worker processes on the same host share the same store instance, but two separate hosts each run their own independent stores. If you need cross-node shared state, see the [Redis module](/advanced/modules).
</Info>
...
```php src/Service/SessionCacheService.php
...
```php src/Service/NotificationDispatcher.php

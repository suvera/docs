...
<Tabs>
  <Tab title="RedisSampleApplication.php">
  ...
  <Tab title="RedisDemoController.php">
...
<Tabs>
  ...
  <Tab title="redis-config.yml">
    ...
    The `__default__` entry supplies shared timeouts. The `primary` connection is also injectable by name with `#[Autowired("primary")]`. See the [Redis module](/modules/data-redis) for clusters, arrays, sentinels, and Dynomite rings.
  </Tab>
</Tabs>
...
- Read the [Redis module](/modules/data-redis) for cluster, array, sentinel, and Dynomite templates.
- Browse all [Libraries](/modules/overview) when you need Kafka, SQS, S3, or Doctrine in the same app.

---
title: "Winter Doctrine: ORM and DBAL Integration Module"
sidebarTitle: "Doctrine"
description: "Configure and use Doctrine ORM and DBAL in Winter Boot. Covers entity managers, connections, declarative and programmatic transactions, and multi-tenant support."
---
...
<Tip>
  Framework datasource, migration, and transaction basics are covered in [Databases and Datasources](/data/database), [Migrations](/data/migrations), and [Transactions](/data/transactions). This page covers Doctrine ORM/DBAL specifics.
</Tip>
...
```php
// ORM - Primary Transaction Manager (defaultdb)
#[Autowired]
...
This module fixes that for you, automatically. Under Swoole, every request
...
<Tip>
  ...
  off without it). This is the kill switch if you ever need the old behavior:
...
4. **One database connection per concurrent request, with a safety cap.**
   Every request that touches the database opens its own real database
   connection, so two requests can never share (and corrupt) one. Requests
   that never touch the database open none, and every connection is closed
   when its request finishes. The log line
   `Doctrine open DB connections: {...}` shows you the live count per pool —
   watch it after deploys.

   Each pool is capped at **50 open connections**. When all 50 are busy, the
   next request waits up to **5 seconds** for one to free up; if none does,
   it fails fast with a clear `PoolExhaustedException` instead of silently

...
Winter SQS is an AWS SQS module for Winter Boot that provides both producer (send) and consumer (receive / process) capabilities. If you already use the [Kafka](/modules/kafka) or [S3](/modules/s3) modules, the configuration patterns here, like named connections and workers, will feel familiar.

## Setup

### Autoload and dependencies

Add the `winter-sqs` directory path to your project's `composer.json` `autoload.psr-4` block:
...
Also make sure the AWS SDK for PHP is installed:
...
<Tip>
  For self-hosted event streaming, see the [Kafka](/modules/kafka) module.
</Tip>
